    ...
    for i in range(len(fs)):
        ...
        for j in range(len(ps)):
            ...
            for k in range(num_trial):
                ...
                res = setUp4D(p=ps[j], freq=fs[i], timeRun = timeRun)
...
def graphPF_Star(p=0.1, freq=0.5, onlineTX = 5.0, onlineTXTime = 1.0, r = 0.01, timeRun = 10.0):
    ...
    surf = ax.plot(fs, costOnFreq[0][0], "r-.")
    surf = ax.plot(fs, costOnFreq[0][1], "b--")
    surf = ax.plot(fs, costOnFreq[1][0], "g--")
    surf = ax.plot(fs, costOnFreq[1][1], "k-.")
